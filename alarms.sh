...
AUTH_TOKEN="Bearer XXXXXXXXXXXXXXXXXXXXXXXXXXXX"

# Create a file to store the responses
OUTPUT_FILE="alarms.txt"

# Iterate through note IDs from 1 to 360
for alarm_id in {358..360}; do
    # Construct the API endpoint with the current note_id
    API_ENDPOINT="$BASE_API_ENDPOINT/$alarm_id/alarms"
    ...
    response=$(curl -i -s -k -X GET \
        ...
        -b "samesite=lax; JSESSIONID=XXXXXXXXXXXXXXXXXXXXXXXXXXXX" \
        "$API_ENDPOINT")
    ...
    echo "--------------------------- Alarm details for Investigation: $alarm_id ---------------------------" >> "$OUTPUT_FILE"

    echo "Processed note ID: $alarm_id"
done
