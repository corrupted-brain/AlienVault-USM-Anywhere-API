...
for attach_id in {0..360}; do
    # Construct the API endpoint with the current note_id
    API_ENDPOINT="$BASE_API_ENDPOINT/$attach_id/attachments"
    ...
    response=$(curl -i -s -k -X GET \
        ...
        -b "samesite=lax; XXXXXXXXXXXXXXXXXXXXX" \
        "$API_ENDPOINT")
    ...
    echo "--------------------------- Attachment for Investigation: $attach_id ---------------------------" >> "$OUTPUT_FILE"

    echo "Processed note ID: $attach_id"
done
