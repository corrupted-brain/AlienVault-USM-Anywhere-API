...
AUTH_TOKEN="Bearer XXXXXXXXXXXXXXXXXXX"
...
for note_id in {0..360}; do
    ...
    API_ENDPOINT="$BASE_API_ENDPOINT/$note_id"
    ...
    response=$(curl -i -s -k -X GET \
        ...
        -b "samesite=lax; JSESSIONID=XXXXXXXXXXXXXXXXXXX" \
        "$API_ENDPOINT")
    ...
    echo "--------------------------- Details of Investigation: $note_id ---------------------------" >> "$OUTPUT_FILE"

    echo "Processed investigation ID: $note_id"
done
